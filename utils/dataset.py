...
class Dataset(data.Dataset):
    def __init__(self, filenames, input_size, params, augment):
        ...
        # Read labels
        labels = self.load_label(filenames, params)
        box, kpt = zip(*labels.values())
        self.box = list(box)
        self.kpt = list(kpt)
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            # Load MOSAIC
            image, box, kpt = self.load_mosaic(index, params)
            # MixUp augmentation
            if random.random() < params['mix_up']:
                index = random.choice(self.indices)
                mix_image1, mix_box1, mix_kpt1 = image, box, kpt
                mix_image2, mix_box2, mix_kpt2 = self.load_mosaic(index, params)

                image, box, kpt = mix_up(mix_image1, mix_box1, mix_kpt1,
                                         mix_image2, mix_box2, mix_kpt2)
        else:
            # Load image
            ...
            kpt = self.kpt[index].copy()
            if box.size:
                box[:, 1:] = wh2xy(box[:, 1:], ratio[0] * w, ratio[1] * h, pad[0], pad[1])
                kpt[..., 0] *= ratio[0] * w
                kpt[..., 1] *= ratio[1] * h
                kpt[..., 0] += pad[0]
                kpt[..., 1] += pad[1]
            if self.augment:
                image, box, kpt = random_perspective(image, box, kpt, params)
        ...
        if self.augment:
            ...
            if random.random() < params['flip_lr']:
                ...
                if nl:
                    box[:, 0] = 1 - box[:, 0]
                    kpt[..., 0] = 1 - kpt[..., 0]
                    kpt = numpy.ascontiguousarray(kpt[:, self.params['flip_index'], :])

        num_kpt = params['kpt_shape'][0]

        target_cls = torch.zeros((nl, 1))
        target_box = torch.zeros((nl, 4))
        target_kpt = torch.zeros((nl, num_kpt, 3))
        if nl:
    ...
    def load_mosaic(self, index, params):
        ...
        for i, index in enumerate(indices):
            ...
            kpt = self.kpt[index].copy()
            if len(box):
                box[:, 1:] = wh2xy(box[:, 1:], shape[1], shape[0], pad_w, pad_h)
                kpt[:, :, 0] *= shape[1]
                kpt[:, :, 1] *= shape[0]
        ...
        # Concat/clip labels
        box4 = numpy.concatenate(box4, 0)
        kpt4 = numpy.concatenate(kpt4, 0)
        ...
        # Augment
        image4, box4, kpt4 = random_perspective(image4, box4, kpt4, params, border)

        return image4, box4, kpt4
        ...
        for filename in filenames:
            try:
                ...
                if image.format.lower() in ('jpg', 'jpeg'):
                    with open(filename, 'rb') as f:
                        ...
                        if f.read() != b'\xff\xd9':  # corrupt JPEG
                            ImageOps.exif_transpose(Image.open(filename)).save(filename, 'JPEG',
                ...
                if os.path.isfile(b.join(filename.rsplit(a, 1)).rsplit('.', 1)[0] + '.txt'):
                    ...
                    nl = len(label)
                    if nl:
                        assert (label[:, 5::3] <= 1).all()
                        assert (label[:, 6::3] <= 1).all()
                        assert label.shape[1] == (5 + num_kpt * num_dim)
                        # All labels
                        ...
                        label = numpy.zeros((0, 5 + num_kpt * num_dim), dtype=numpy.float32)  # label empty
                else:
                    label = numpy.zeros((0, 5 + num_kpt * num_dim), dtype=numpy.float32)  # label missing
                box = label[:, :5]
                kpt = label[:, 5:].reshape(-1, num_kpt, num_dim)
                if num_dim == 2:
                    kpt_mask = numpy.ones(kpt.shape[:2], dtype=numpy.float32)
                    kpt_mask = numpy.where(kpt[..., 0] < 0, 0.0, kpt_mask)
                    kpt_mask = numpy.where(kpt[..., 1] < 0, 0.0, kpt_mask)
                    kpt = numpy.concatenate([kpt, kpt_mask[..., None]], axis=-1)  # (nl, num_kpt, 3)
                x[filename] = [box, kpt]
...
def random_perspective(image, box, kpt, params, border=(0, 0)):
    ...
    if n:
        xy = numpy.ones((n * 4, 3))
        xy[:, :2] = box[:, [1, 2, 3, 4, 1, 4, 3, 2]].reshape(n * 4, 2)  # x1y1, x2y2, x1y2, x2y1
        ...
        n, num_kpt = kpt.shape[:2]
        if n != 0:
            xy = numpy.ones((n * num_kpt, 3))
            visible = kpt[..., 2].reshape(n * num_kpt, 1)
            xy[:, :2] = kpt[..., :2].reshape(n * num_kpt, 2)
            xy = xy @ matrix.T  # transform
            xy = xy[:, :2] / xy[:, 2:3]  # affine
            mask = (xy[:, 0] < 0) | (xy[:, 1] < 0) | (xy[:, 0] > w) | (xy[:, 1] > h)
            visible[mask] = 0
            kpt = numpy.concatenate([xy, visible], axis=-1).reshape(n, num_kpt, 3)
        # clip
        new[:, [0, 2]] = new[:, [0, 2]].clip(0, w)
        new[:, [1, 3]] = new[:, [1, 3]].clip(0, h)
        kpt[..., 0] = kpt[..., 0].clip(0, w)
        kpt[..., 1] = kpt[..., 1].clip(0, w)
        # filter candidates
        indices = candidates(box1=box[:, 1:5].T * s, box2=new.T)
...
def mix_up(image1, box1, kpt1, image2, box2, kpt2):
    ...
    kpt = numpy.concatenate((kpt1, kpt2), 0)
    return image, box, kpt
