...
class Head(torch.nn.Module):
    ...
    def __init__(self, nc=80, kpt_shape=(14, 3), ch=()):
        super().__init__()
        ...
        self.cls = torch.nn.ModuleList(torch.nn.Sequential(Conv(x, c2, 3),
                                                           Conv(c2, c2, 3),
        ...
        self.kpt = torch.nn.ModuleList(torch.nn.Sequential(Conv(x, c3, 3),
                                                           Conv(c3, c3, 3),
    ...
    def forward(self, x):
        ...
        if self.training:
            return x_det, x_kpt
        return torch.cat([x_det, self.decode_kpt(x_kpt)], dim=1)
...
class YOLO(torch.nn.Module):
    def __init__(self, width, depth, num_classes):
        ...
        self.head = Head(num_classes, (14, 3), (width[3], width[4], width[5]))
